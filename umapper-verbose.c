...
void print_device(mapper_db_device dev, int details){
   ...
   char type;
   const void *val;
   while (!mapper_db_device_property_index(dev, i++, &key, &type, &val, &len)){
      printf(", %s=", key);
...
void print_connection(mapper_db_connection con, int details){
   ...
   if(!details){
    ...
    char type;
    const void *val;
    while(!mapper_db_connection_property_index(con, i++, &key, &type, &val, &len)){
...
void print_signal(mapper_db_signal sig, int details){
   ...
   if(!details){
    ...
    char type;
    const void *val;
    while(!mapper_db_signal_property_index(sig, i++, &key, &type, &val, &len))
    {
        die_unless(val!=0, "returned zero value\n");
...
void print_link(mapper_db_link lnk, int details){
   ...
   char type;
   const void *val;
   while (!mapper_db_link_property_index(lnk, i++, &key, &type, &val, &len)){
      printf(", %s=", key);
      lo_arg_pp(type, (lo_arg*)val);
   }
   printf("\n");
